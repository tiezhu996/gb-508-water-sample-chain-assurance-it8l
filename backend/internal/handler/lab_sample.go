...
func (h *LabSampleHandler) Register(group *gin.RouterGroup) {
	...
	resource.POST("/:id/handover", middleware.RequireMinimumRole("operator"), h.handover)
...
func (h *LabSampleHandler) handover(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.HandoverLabSample
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
	item, err := h.service.Handover(c.Request.Context(), id, input, actorFromContext(c), requestIDFromContext(c))
	if err != nil {
		handleError(c, err)
		return
	}
