...
- 实验室样本支持保管交接：`POST /api/samples/:id/handover` 仅当前保管人可提交，目标账号必须启用且具备 operator 以上角色；样本已处置、目标与当前保管人相同、账号不存在/停用或版本过期一律拒绝且原记录不变，乐观锁保证并发交接只成功一次，成功后同步保管人、交接时间与版本并写入 `handover` 审计。
