...
export async function handoverLabSample(id: number, targetUsername: string, expectedVersion: number, reason: string) {
  return request<DomainRecord>(`/samples/${id}/handover`, {
    method: 'POST', body: JSON.stringify({ targetUsername, expectedVersion, reason }),
  });
}
