...
export class EntityStore {
  ...
  async handover(path: string, item: DomainRecord, targetUsername: string, remark: string) { this.patch({ loading: true, error: '' }); try { await request<DomainRecord>(`/${path}/${item.id}/handover`, { method: 'POST', body: JSON.stringify({ targetUsername, expectedVersion: item.version, remark }) }); await this.load(path); } catch (error) { this.patch({ loading: false, error: error instanceof Error ? error.message : String(error) }); throw error; } }
